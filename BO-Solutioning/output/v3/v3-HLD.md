...
* Agent-friendly read-only coverage visibility and Home Connect UI presentation driven by form-governance rules
...
* External dependency: Home Connect integration feed for Appliance ID, eligibility indicator, and Connection Status
...
|| Acceptance Criterion || FR to Design Mapping || Solution || Complexity || Details ||
| AC-FR-4 - Show newly created appliance in Customer Appliance / Customer Asset list | FR-4 | {status:colour=Green|title=COOB} | {status:colour=Yellow|title=Medium} | • Refresh customer asset subgrid after successful create so the new appliance is immediately visible in context \\ • Keep list and detail navigation consistent from account/contact to appliance record \\ • Validate behavior with positive, negative, and role-based scenarios |
...
| DDP-63819 | Home Connect Eligibility Visibility | AC1.1 - Show Home Connect fields for eligible appliances | {status:colour=Green|title=COOB} | {status:colour=Yellow|title=Medium} |
| DDP-63819 | Home Connect Eligibility Visibility | AC1.2 - Hide Home Connect fields for non-enabled appliances | {status:colour=Green|title=COOB} | {status:colour=Yellow|title=Medium} |
...
| DDP-63819 | Home Connect ID Governance | AC2.2 - Keep Home Connect Appliance ID read-only | {status:colour=Green|title=COOB} | {status:colour=Yellow|title=Medium} |
...
| DDP-63819 | Home Connect Status Mapping | AC3.2 - Suppress status field and icon for non-enabled appliances | {status:colour=Green|title=COOB} | {status:colour=Yellow|title=Medium} |
...
The implementation is centered on Dynamics 365 Customer Service and Dataverse with Customer Asset as the appliance anchor and Entitlement as coverage source. Low-code automation handles notification orchestration, Customer Service form configuration governs Home Connect UI visibility and read-only behavior, and integration patterns support Home Connect identifier retrieval plus status visibility. Deprecated Contract table usage is not used for new design decisions in this pass.
